export default async function handler(req, res) {
  ...
  try {
    // Menggunakan Gemini API (Pastikan GEMINI_API_KEY sudah ada di Vercel)
    ...
    const prompt = `Pengguna melakukan: "${activities}". Berikan rekomendasi aktivitas selanjutnya dan alasan ramah. Balas WAJIB dalam format JSON murni: {"rekomendasi": "...", "tanggapan": "..."}`;
    ...
    if (!response.ok) throw new Error(data.error?.message || "Gagal menghubungi Gemini");
